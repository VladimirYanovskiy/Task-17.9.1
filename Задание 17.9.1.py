...
if " " not in sequence_numbers:
    print("\nЧисла введены без пробелов (введите числа, согласно условиям ввода).")
    sequence_numbers = input('Введите целые числа через пробел: ')
if not is_int(sequence_numbers):
    print('\nВведено не число, либо не целое число (введите целое число).\n')
    print(error)
else:
...
print(f'Сортировка списка по возрастанию элементов в нем: {list_sequence_numbers}')
...
if not binary_search(list_sequence_numbers, user_number, 0, len(list_sequence_numbers)):
    ...
    min_ind = ind - 1
    if rI < user_number:
        print(f'''В списке нет введенного элемента: {user_number} 
